void System_Processes(){
  ...
  if(enableFan==true){
    if(enableDynamicCooling==false){                                   //STATIC PWM COOLING MODE (2-PIN FAN - no need for hysteresis, temp data only refreshes after 'avgCountTS' or every 500 loop cycles)                       
      ...
      else if(temperature<(temperatureFan-1)){fanStatus=0;}            //Turn off fan when set fan temp reached
  ...
  if(LVD==true){
    if(voltageOutput>=LVDBatteryMax){digitalWrite(LVD_pin,HIGH);}
    else if(voltageOutput<=LVDBatteryMin){digitalWrite(LVD_pin,LOW);}
    // digitalWrite(LVD_pin,LVDStatus);
  }
  else{digitalWrite(LVD_pin,LOW);}
  //////////// LOOP TIME STOPWATCH ////////////
...
void factoryReset(){
  ...
  EEPROM.write(1,60);   //STORE: Max Battery Voltage (whole)
  EEPROM.write(2,0);    //STORE: Max Battery Voltage (decimal)
  EEPROM.write(3,10);    //STORE: Min Battery Voltage (whole)
  ...
  EEPROM.write(14,75);  //STORE: Out div Ratio (whole)
  EEPROM.write(15,0);   //STORE: Out div Ratio (decimal)
  EEPROM.write(16,40);  //STORE: In div Ratio (whole)
  EEPROM.write(17,215); //STORE: In div Ratio (decimal)
  EEPROM.write(18,1);   //STORE: LVD
  EEPROM.write(19,80);  //STORE: LVD Max Battery (whole)
  EEPROM.write(20,0);   //STORE: LVD Max Battery (decimal)
  EEPROM.write(21,22);  //STORE: LVD Min Battery (whole)
  EEPROM.write(22,0);   //STORE: LVD Min Battery (decimal)
  EEPROM.commit();
  loadSettings();
}
...
void loadSettings(){ 
  ...
  outVoltageDivRatio = EEPROM.read(14)+(EEPROM.read(15)*.01);// Load saved out voltage divider ratio
  inVoltageDivRatio  = EEPROM.read(16)+(EEPROM.read(17)*.01);// Load saved in voltage divider ratio
  LVD                = EEPROM.read(18);                      // Load saved LVD Enable
  LVDBatteryMax      = EEPROM.read(19)+(EEPROM.read(20)*.01);// Load saved LVD Max Battery
  LVDBatteryMin      = EEPROM.read(21)+(EEPROM.read(22)*.01);// Load saved LVD Min Battery
}
...
//EEPROM.write(11,flashMemLoad);       //STORE: Enable autoload (must be excluded from bulk save, uncomment under discretion)
  ...
  conv1 = outVoltageDivRatio*100;      //STORE: output voltage divider ratio (gets whole number)
  conv2 = conv1%100;                   //STORE: output voltage divider ratio (gets decimal number and converts to a whole number)
  EEPROM.write(14,outVoltageDivRatio);
  EEPROM.write(15,conv2);
  conv1 = inVoltageDivRatio*100;       //STORE: input voltage divider ratio (gets whole number)
  conv2 = conv1%100;                   //STORE: input voltage divider ratio (gets decimal number and converts to a whole number)
  EEPROM.write(16,inVoltageDivRatio);
  EEPROM.write(17,conv2);
  EEPROM.write(18,LVD);
  conv1 = LVDBatteryMax*100;           //STORE: LVD Max Battery (gets whole number)
  conv2 = conv1%100;                   //STORE: LVD Max Battery (gets decimal number and converts to a whole number)
  EEPROM.write(19,LVDBatteryMax);
  EEPROM.write(20,conv2);
  conv1 = LVDBatteryMin*100;           //STORE: LVD Min Battery (gets whole number)
  conv2 = conv1%100;                   //STORE: LVD Min Battery (gets decimal number and converts to a whole number)
  EEPROM.write(21,LVDBatteryMin);
