...
void Read_Sensors(){
  ...
  for(int i = 0; i<avgCountCS; i++){
    ...
    // CSO = CSO + ads.computeVolts(ads.readADC_SingleEnded(0));
  }
  CSI_converted = (CSI/avgCountCS)*1.3300;
  currentInput  = (CSI_converted-InCurrentMidPoint)/InCurrentSensV; // (CSI_converted-currentMidPoint)/0.0165;
  // CSO_converted = (CSO/avgCountCS)*1.3300;
  // currentOutput  = (CSO_converted-OutCurrentMidPoint)/OutCurrentSensV; // (CSI_converted-currentMidPoint)/0.0165;
  if(currentInput<0){currentInput=0.0000;}
  if(voltageOutput<=0){currentOutput = 0.0000;}
  else{currentOutput = powerOutput/voltageOutput;}// ((voltageInput*currentInput)/voltageOutput);}
  ...
  if(buckEnable==0 && FLV==0 && OOV == 0){
    ...
    // CSO = 0.0000;      //Clear Previous Current
    for(int i = 0; i<avgCountCS; i++){
    CSI = CSI + ads.computeVolts(ads.readADC_SingleEnded(2));
    // CSO = CSO + ads.computeVolts(ads.readADC_SingleEnded(0));
  }
    InCurrentMidPoint = ((CSI/avgCountCS)*1.3300)-0.003;
    // OutCurrentMidPoint= ((CSO/avgCountCS)*1.3300);//-0.003;
  }
