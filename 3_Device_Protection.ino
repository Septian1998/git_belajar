void backflowControl(){                                                //PV BACKFLOW CONTROL (INPUT MOSFET) 
  ...
  // currentMillisBackflow = millis();
  digitalWrite(relay_PV,bypassEnable);
  // while(millis() < currentMillisBackflow + 2000){/*wait approx. [period] ms*/}
  digitalWrite(relay_BAT,bypassEnable);
  // digitalWrite(relay_PV,bypassEnable);
  // digitalWrite(relay_BAT,bypassEnable);
}
...
void Device_Protection(){
  ...
  // if(currentOutput>0.3)                                    {IUC=1;ERR++;errorCount++;}else{IUC=0;}  //IUC - INPUT UNDER CURRENT
  ...
  if(output_Mode==0){                                                                               //PSU MODE specific protection protocol
    ...
    // if(voltageInput>(voltageInputPrev1+10) && currentInput<=0.5) {IUC=1;ERR++;REC=1;}else{IUC=0;}
    // if(currentInput<0.0 && voltageInput>30)  {IUC=1;ERR++;REC=1;}else{IUC=0;}        //IUC - INPUT UNDER CURRENT
  }
}